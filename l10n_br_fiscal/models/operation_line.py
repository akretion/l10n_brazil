...
class OperationLine(models.Model):
    ...
    add_to_amount = fields.Boolean(
        string="Add to Document Amount?", default=True
    )
    ...
    def get_document_type(self, company):
        ...
        if self.document_type_id:
            ...
            if not company.document_type_id:
                raise UserError(
                    _(
                        "You need set a default fiscal document "
                        "in your company !"
                    )
                )
    ...
    def map_fiscal_taxes(
        ...
        mapping_result = {"taxes": {}, "cfop": False, "taxes_value": 0.00}
        ...
        if company.tax_framework == TAX_FRAMEWORK_NORMAL:
            ...
            for tax in partner.fiscal_profile_id.tax_definition_ids.mapped(
                "tax_id"
            ):
                mapping_result["taxes"][tax.tax_domain] = tax
