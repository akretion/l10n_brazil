...
class DocumentInvalidateNumber(models.Model):
    ...
    invalid_number_document_event_ids = fields.One2many(
        "l10n_br_fiscal.document.event",
        "invalid_number_document_event_id",
        u"Eventos",
            ...
            if record.company_id:
                ...
                if self.search_count(domain):
                    raise ValidationError(
                        _("Não é permitido faixas sobrepostas!")
                    )
        return True
    ...
    def unlink(self):
        ...
        for invalid_number in self:
            if invalid_number["state"] in ("draft"):
                ...
                raise UserError(
                    _(u"Você não pode excluir uma sequência concluída.")
                )
    ...
    def invalidate(self, event_id):
        for record in self:
            ...
            if record.document_id:
                ...
                for number in range(
                    record.number_start, record.number_end + 1
                ):
