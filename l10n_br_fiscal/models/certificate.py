...
class Certificate(models.Model):
    ...
    issuer_name = fields.Char(
        string="Issuer", size=120, readonly=True, store=True
    )

    owner_name = fields.Char(
        string="Owner", size=120, readonly=True, store=True
    )

    owner_cnpj_cpf = fields.Char(
        string="CNPJ/CPF", size=18, readonly=True, store=True
    )
    ...
    is_valid = fields.Boolean(
        compute="_compute_is_valid", string="Is Valid?", store=False
    )
