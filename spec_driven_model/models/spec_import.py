...
class AbstractSpecMixin(models.AbstractModel):
        ...
        if (
            ...
            if attr.get_container() == 0:
                ...
                for line in [li for li in value if li]:
                    line_vals = comodel.build_attrs(line, path=child_path, defaults_model=comodel)
    ...
    @api.model
    def _prepare_import_dict(self, vals, model=None, parent_dict=None, defaults_model=False):
        """
    ...
    def match_record(self, rec_dict, parent_dict, model=None):
        ...
        for key in keys:
            if rec_dict.get(key):
                ...
                if match_ids:
                    if len(match_ids) > 1:
                        _logger.warning("!! WARNING more than 1 record found!! model: %s, domain: %s" % (model, domain))
