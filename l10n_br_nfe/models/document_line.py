...
> <det>
    ...
    def _build_many2one(self, comodel, vals, new_value, key, value, path):
        ...
        if key == "nfe40_ISSQN":
            ...
            self._import_tax_attrs(
                "pis",  # (tags suffix are *PIS rather than *PISST)
                value,
                new_value,
            )
            ...
            self._import_tax_attrs(
                "cofins",  # (tags suffix are *COFINS rather than *COFINSST)
                value,
                new_value,
            )
    ...
    def _import_tax_attrs(self, kind, value, odoo_attrs, sub_tags=None):
        """
        ...
        if cst:
            odoo_attrs["%s_cst_id" % (kind,)] = self.env.ref(
        ...
        percent = map_binding_attr("p%s" % (kind.upper(),), "%s_percent" % (kind,))
        if percent:
            tax_group_id = self.env.ref("l10n_br_fiscal.tax_group_%s" % (kind,)).id
        ...
        if kind in ("icms", "icmsufdest"):
            ...
            map_binding_attr("pRedBC", "icms_reduction")
